...
const folders = [
  ...
  { id: "drafts", label: "Drafts", count: 3, icon: Archive },
  { id: "sent", label: "Sent", count: 892, icon: Send },
  { id: "scheduled", label: "Scheduled", count: 12, icon: Bell },
...
const emails = [
  ...
  { id: 4, from: "Team Zegoro", email: "team@zeg.steinhoff.group", subject: "Dashboard v2 deployment live on zeg.steinhoff.group", preview: "The new Zegoro Dashboard is live. All subsystems are accessible: Dashboard, Email, Docs, Files, Obsidian, Calendar, Settings, Auth. Check it out and let us know what you think.", time: "Yesterday", unread: false, starred: true, label: "steinhoff", avatar: "Z" },
...
const initState = {
  ...
  subject: "",
  body: "",
  selectedFolder: "inbox" as string,
...
export default function EmailView() {
  ...
  const filteredEmails = emails.filter((e) => {
    if (state.selectedFolder === "inbox") return e.unread
    if (state.selectedFolder === "drafts") return e.subject.toLowerCase().includes("draft")
    if (state.selectedFolder === "sent") return !e.unread
    if (state.selectedFolder === "archived" || state.selectedFolder === "scheduled") return false
    return true
  }).filter((e) => {
  ...
  function markRead(id: number) {
    setState((s) => ({ ...s, selectedEmailId: id }))
  }

  function handleSend() {
    if (!state.to || !state.body) return
    setState((s) => ({ ...s, composing: false, to: "", subject: "", body: "" }))
      ...
      <div className="flex h-full gap-0">
        ...
        <div className="w-52 border-r border-border bg-card flex flex-col shrink-0">
          ...
          <ScrollArea className="flex-1">
              ...
              {folders.map((f) => {
                ...
                return (
                  <Button
                    ...
                    className={`w-full justify-start gap-2 h-8 text-sm ${isActive ? "bg-accent text-accent-foreground" : ""}`}
            ...
            <div className="p-2">
              ...
              <div className="space-y-0.5">
                {labels.map((l) => (
                  <button
                    key={l.id}
                    className={`w-full flex items-center gap-2 px-2 py-1 rounded text-sm transition-colors ${state.selectedLabel === l.id ? "bg-accent text-accent-foreground" : "hover:bg-muted"}`}
            ...
            <div className="p-2">
              <Button variant="default" className="w-full justify-center gap-1.5" onClick={() => setShowCompose(true)}>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div className="flex items-center gap-2 p-3 border-b bg-card">
            <Select value={state.selectedFolder} onValueChange={(v) => setState((s) => ({ ...s, selectedFolder: v, selectedEmailId: null }))}>
            ...
            <div className="flex-1 flex items-center gap-2 ml-2">
              ...
              <Button variant="outline" size="icon" onClick={() => setState((s) => ({ ...s, viewMode: s.viewMode === "list" ? "grid" : "list" }))}>
                {state.viewMode === "list" ? <Grid className="h-4 w-4" /> : <List className="h-4 w-4" />}
              </Button>
              <Button variant="outline" size="icon" onClick={() => setState((s) => ({ ...s, searchQuery: "" }))}>
                ...
                <div className="divide-y">
                  {filteredEmails.length === 0 ? (
                    ...
                    filteredEmails.map((email) => (
                      <div
                        key={email.id}
                        className={`p-3 cursor-pointer transition-colors ${state.selectedEmailId === email.id ? "bg-accent/50 border-l-2 border-l-primary" : "hover:bg-muted/50"} ${email.unread ? "bg-muted/30" : ""}`}
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-2">
                              <div className="flex items-center gap-1.5 min-w-0">
                                ...
                                <span className={`text-sm truncate ${email.unread ? "font-semibold" : "font-normal"}`}>{email.from}</span>
                              </div>
                            ...
                            <p className={`text-sm truncate ${email.unread ? "font-semibold" : "font-normal text-muted-foreground"}`}>{email.subject}</p>
                            <p className="text-xs text-muted-foreground truncate mt-0.5">{email.preview}</p>
                            {email.label && (
                              <Badge variant="secondary" className={`mt-1.5 text-[10px] ${email.label === "important" ? "text-red-500 bg-red-500/10" : ""}`}>
                ...
                <>
                  <div className="p-4 border-b">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <Button variant="ghost" size="icon" onClick={() => setState((s) => ({ ...s, selectedEmailId: null }))}>
                        <X className="h-4 w-4" />
                      </Button>
                    </div>
                  </div>
                  ...
                  <ScrollArea className="flex-1 p-4">
                    ...
                    <p className="text-sm leading-relaxed whitespace-pre-wrap mt-4 text-muted-foreground">
                      —{selectedEmail.from}<br />{selectedEmail.time}
                    </p>
                  </ScrollArea>

                  <div className="p-3 border-t bg-card flex items-center gap-1 flex-wrap">
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button variant="ghost" size="icon" onClick={() => { setReplyTo(selectedEmail.email); setShowReply(true) }}>
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button variant="ghost" size="icon" onClick={() => setShowForward(true)}>
          ...
          <div className="space-y-3">
            <div className="flex gap-2">
              <div className="flex-1">
                ...
                <Input value={state.to} onChange={(e) => setState((s) => ({ ...s, to: e.target.value }))} placeholder="recipient@email.com" className="mt-0.5 h-7 text-xs" />
              </div>
              <div className="flex-1">
                <Label className="text-xs">Subject</Label>
                <Input value={state.subject} onChange={(e) => setState((s) => ({ ...s, subject: e.target.value }))} placeholder="Subject line" className="mt-0.5 h-7 text-xs" />
              </div>
            </div>
            <Textarea value={state.body} onChange={(e) => setState((s) => ({ ...s, body: e.target.value }))} placeholder="Write your message..." rows={8} className="h-48 text-sm" />
          </div>
          <DialogFooter className="gap-2 sm:gap-0">
            <Button variant="outline" size="sm" onClick={() => setShowCompose(false)}>Discard</Button>
            <Button size="sm" onClick={handleSend} disabled={!state.to || !state.body} className="gap-1.5">
              <Send className="h-3.5 w-3.5" />
              Send
            </Button>
          </DialogFooter>
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              <Label className="text-xs">To</Label>
              <Input value={replyTo} readOnly className="h-7 text-xs bg-muted" />
            </div>
            <Textarea value={replyText} onChange={(e) => setReplyText(e.target.value)} placeholder="Write your reply..." rows={6} className="h-40 text-sm" />
          </div>
          <DialogFooter className="gap-2 sm:gap-0">
            <Button variant="outline" size="sm" onClick={() => setShowReply(false)}>Discard</Button>
            <Button size="sm" onClick={handleReply} disabled={!replyText} className="gap-1.5">
              <Send className="h-3.5 w-3.5" />
              Reply
            </Button>
          </DialogFooter>
      ...
      <Dialog open={showForward} onOpenChange={setShowForward}>
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              <Label className="text-xs">To</Label>
              <Input value={forwardSubject} onChange={(e) => setForwardSubject(e.target.value)} placeholder="recipient@email.com" className="mt-0.5 h-7 text-xs" />
            </div>
            {forwardSubject && (
            ...
            <Textarea value={forwardText} onChange={(e) => setForwardText(e.target.value)} placeholder="Add a message..." rows={6} className="h-40 text-sm" />
          </div>
          <DialogFooter className="gap-2 sm:gap-0">
            <Button variant="outline" size="sm" onClick={() => setShowForward(false)}>Discard</Button>
            <Button size="sm" onClick={handleForward} disabled={!forwardText} className="gap-1.5">
